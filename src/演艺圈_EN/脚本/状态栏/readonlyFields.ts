...
export function setupReadonlyFields(): void {
  ...
  eventOn(
    ...
    (new_variables: Record<string, unknown>, old_variables: Record<string, unknown>) => {
      ...
      const old_receivables = _.get(old_stat_data, 'companyAccount.$receivablesByDueMonth', {}) as Record<
        string,
        number
      >;
      ...
      if (old_company_cash !== undefined) {
        ...
        const currentYM: string = currentYMMatch
          ? `${currentYMMatch[1]}-${currentYMMatch[2]}`
      ...
      const new_running_projects = _.get(new_stat_data, 'companyAccount.monthlyRevenueSources') as
        | Record<string, ProjectWithProfit>
        | undefined;
