...
// 简单的 waitUntil 实现（如果全局不可用）
const waitUntil = async (condition: () => boolean, options?: { timeout?: number; interval?: number }) => {
...
const getVal = (data: any, path: string, def: any = '无'): any => {
  if (!data) return def;
...
$(
  errorCatched(async () => {
    ...
    const waitUntilFn = (window as any).waitUntil || waitUntil;
    await waitUntilFn(() => {
      try {
        if (window.Mvu && typeof window.Mvu.getMvuData === 'function') {
          const variables = window.Mvu.getMvuData({ type: 'message', message_id: 'latest' });
